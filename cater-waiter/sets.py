...
def check_drinks(drink_name, drink_ingredients):
    ...
    The function should return the name of the drink followed
    by "Mocktail" (non-alcoholic) and drink
...
def categorize_dish(dish_name, dish_ingredients):
    ...
    This function should return a string with the `dish name:
    <CATEGORY>` (which meal category the dish belongs to).
...
def tag_special_ingredients(dish):
    ...
    Return the dish name followed by the `set` of
    ingredients that require a special note on the dish description.
    For the purposes of this exercise,
    all allergens or special ingredients that need to be tracked are in the
    SPECIAL_INGREDIENTS constant imported from `sets_categories_data.py`.
    """
    name_of_dish, ingredients = dish
    ingredients = set(ingredients)

    maybe_allegric_ingredients = ingredients.intersection(SPECIAL_INGREDIENTS)

    return (name_of_dish, maybe_allegric_ingredients)
...
def compile_ingredients(dishes):
    ...
    compiled_items = set()
    for item in dishes:
        compiled_items = compiled_items | item
...
def separate_appetizers(dishes, appetizers):
    ...
    only_dishes = dishes.difference(appetizers)
...
def singleton_ingredients(dishes, intersection):
    ...
    :param intersection: constant - can be one of
    `<CATEGORY>_INTERSECTIONS` constants imported from `sets_categories_data.py`.
    :return: set - containing singleton ingredients.

    Each dish is represented by a `set` of its ingredients.

    Each `<CATEGORY>_INTERSECTIONS` is an `intersection` of all
    dishes in the category. `<CATEGORY>` can be any one of:
        (VEGAN, VEGETARIAN, PALEO, KETO, or OMNIVORE).

    The function should return a `set` of ingredients that only appear in a single dish.
    """
    # I didn't understand the question to be honest here, this is stolen. But I'm an honest thief.
